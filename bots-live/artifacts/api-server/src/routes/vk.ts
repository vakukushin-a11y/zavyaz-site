...
async function fetchAllAiNews(langFilter: string): Promise<AiNewsItem[]> {
  const sources = langFilter === "all"
    ? AI_NEWS_SOURCES
    : AI_NEWS_SOURCES.filter((s) => s.lang === langFilter);
  ...
  if (failedCount > 0) {
    logger.warn({ failedCount, langFilter }, "Some AI news RSS sources failed");
  }
...
vkRouter.get("/vk/ai-news", async (req, res) => {
  try {
    ...
    const langFilter = (req.query["lang"] as string) ?? "all";
    const cacheKey = langFilter;
    ...
    if (!forceRefresh && cached && now - cached.fetchedAt < CACHE_TTL_MS) {
      res.json({ items: cached.items.slice(0, limit), cached: true });
      return;
    }
    ...
    if (cached && !forceRefresh) {
      res.json({ items: cached.items.slice(0, limit), cached: true });
      fetchAllAiNews(langFilter)
        .then((items) => newsCache.set(cacheKey, { items, fetchedAt: Date.now() }))
        .catch((err) => logger.warn({ err }, "Background AI news refresh failed"));
      return;
    }

    // No cache — fetch fresh
    const items = await fetchAllAiNews(langFilter);
    newsCache.set(cacheKey, { items, fetchedAt: Date.now() });
    res.json({ items: items.slice(0, limit), cached: false });
  } catch (err) {
    req.log.error({ err }, "GET /api/vk/ai-news failed");
...
export function warmVkNewsCache(): void {
  fetchAllAiNews("all")
    .then((items) => {
      newsCache.set("all", { items, fetchedAt: Date.now() });
      logger.info({ count: items.length }, "VK AI news cache warmed");
    })
    .catch((err) => logger.warn({ err }, "VK AI news cache warm-up failed"));
}
