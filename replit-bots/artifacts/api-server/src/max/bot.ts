...
class MaxBot {
  ...
  // ── Image sending via public URL ─────────────────────────────────────────────

  /** Send up to maxCount product photos by constructing their public API URLs. */
  private async sendPhotos(userId: number, files: string[], maxCount = 3): Promise<void> {
    const existing = files.filter((f) => fs.existsSync(f)).slice(0, maxCount);
    if (existing.length === 0) return;

    // Resolve the public domain (prod first, then dev)
    const domain =
      (process.env.REPLIT_DOMAINS ?? "").split(",").find(Boolean) ||
      process.env.REPLIT_DEV_DOMAIN;
    if (!domain) {
      logger.warn("No Replit domain found — cannot send product photos in MAX");
      return;
    }

    for (const filePath of existing) {
      // Convert absolute path to URL-safe relative path under /api/products-thumb/
      // This endpoint resizes and recompresses images so MAX API can accept them.
      const rel = path.relative(PRODUCTS_DIR, filePath); // e.g. "Кардиганы/Фотоизображения/image1.png"
      const encoded = rel.split(path.sep).map(encodeURIComponent).join("/");
      const url = `https://${domain}/api/products-thumb/${encoded}`;
  ...
  private async sendNewsDigest(userId: number): Promise<void> {
    ...
    if (articles.length === 0) {
      // No fresh news — fall back to most recent available articles
      articles = await db
        .select()
        .from(knitwearNewsTable)
    ...
    if (articles.length === 0) {
      await this.sendMessage(userId, "📰 В базе пока нет новостей о трикотаже — загляните позже!", this.buildMainMenuKeyboard());
      return;
    }

    const header = isFallback
      ? `📰 Свежих новостей за 7 дней нет, но вот самые последние из архива (${articles.length} материалов):`
      : `📰 Новости трикотажной индустрии — за 7 дней (${articles.length} материалов):`;
    await this.sendMessage(userId, header);
    ...
    for (const article of articles) {
      ...
      const rawContent = [article.title, article.aiAnalysis ?? article.summary ?? ""].filter(Boolean).join("\n\n");
      let analysis = "";
      try {
        ...
        analysis = article.aiAnalysis ?? article.summary ?? "";
      }
      ...
      await this.sendMessage(userId, parts);

      const imgUrl = article.imageUrl ?? await fetchOgImage(article.url);
      if (imgUrl) {
        await this.post("/messages", { user_id: userId }, {
          attachments: [{ type: "image", payload: { url: imgUrl } }],
        });
      }
    }

    await this.sendMessage(userId, "Это все актуальные новости. Чем ещё могу помочь?", this.buildMainMenuKeyboard());
  }
  ...
  private async sendLogoThenMenu(userId: number, menuText: string): Promise<void> {
    const logoUrl = `https://${this.getPublicDomain()}/welcome/logo.png`;
    await this.post("/messages", { user_id: userId }, {
